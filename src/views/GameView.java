...
public class GameView extends JPanel {
    ...
    private final JLabel shuffleTimerLabel = new JLabel("셔플까지 남은 시간: 100");
    ...
    public GameView(int size) {
        ...
        shuffleTimerLabel.setForeground(Color.WHITE);
        shuffleTimerLabel.setFont(new Font("궁서 보통", Font.BOLD, 16));
        shuffleTimerLabel.setVisible(false); // 기본적으로 숨김
        ...
        backButton.setPreferredSize(new Dimension(100, 30)); // 버튼 크기 조정
        topRightPanel.add(backButton, BorderLayout.EAST);

        JPanel topLeftPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        topLeftPanel.setOpaque(false);
        topLeftPanel.add(timerLabel);
        topLeftPanel.add(shuffleTimerLabel);
        topLeftPanel.add(moveLabel);
    ...
    public void updateView(Tile[] tiles) {
        for (int i = 0; i < tiles.length; i++) {
            if (tiles[i].getNumber() == 0) {
                ...
                buttons[i].setIcon(customTileIcons != null ? customTileIcons[tiles[i].getNumber() - 1] : defaultTileIcon);
            }
        }
    }
    ...
    public void setImpossibleMode(boolean isImpossibleMode) {
        ...
        timerLabel.setVisible(!isImpossibleMode);
        shuffleTimerLabel.setVisible(isImpossibleMode);
        revalidate();
        repaint();
    }
    ...
    public void setMoveLimitMode(boolean isMoveLimitMode) {
        ...
        if (isMoveLimitMode) {
            moveLabel.setText("남은 이동 횟수: 30");
        } else {
            moveLabel.setText("이동 횟수: 0");
        }
        revalidate();
        repaint();
    }
    ...
    public void updateMoveCount(int moveCount) {
        if (isMoveLimitMode) {
            moveLabel.setText("남은 이동 횟수: " + moveCount);
        } else {
            moveLabel.setText("이동 횟수: " + moveCount);
        }
    }

    public void updateShuffleTimer(int seconds) {
        if (isImpossibleMode) {
            shuffleTimerLabel.setText("셔플까지 남은 시간: " + seconds);
        }
    }
